...
const checkOwnerAuth = async (req, res, next) => {
    if (!req.session.userId) return res.status(403).json({ message: 'Forbidden: Owner access required' });
    const user = await userModel.findById(req.session.userId);
    if (!user || user.role !== 'owner') return res.status(403).json({ message: 'Forbidden: Owner access required' });
    req.user = user;
    next();
};

const checkAdminAuth = async (req, res, next) => {
    if (!req.session.userId) return res.status(403).json({ message: 'Forbidden: Admin access required' });
    const user = await userModel.findById(req.session.userId);
...
const checkModAuth = async (req, res, next) => {
    if (!req.session.userId) return res.status(403).json({ message: 'Forbidden: Moderator access required' });
    const user = await userModel.findById(req.session.userId);
    if (!user || user.role !== 'moderator') return res.status(403).json({ message: 'Forbidden: Moderator access required' });
    req.user = user;
    next();
};
...
const getUserRank = (role) => {
    const ranks = { guest: 1, user: 2, moderator: 3, admin: 4, owner: 5 };
    return ranks[role] || 0;
};
...
app.post('/api/v1/auth/login', async (req, res) => {
    ...
    if (user.status === 'banned') return res.status(403).json({ message: 'Your account has been blocked.' });
...
app.post('/api/owner/login', async (req, res) => {
    ...
    if (!owner || !(await bcrypt.compare(password, owner.password))) return res.status(401).json({ message: 'Invalid owner credentials' });
    if (owner.status === 'banned') return res.status(403).json({ message: 'Your account has been blocked.' });
...
app.post('/api/admin/min', async (req, res) => {
    const { email, password } = req.body;
    if (!email || !password) return res.status(400).json({ message: 'Missing email or password' });
    const admin = await userModel.findOne({ email });
    if (!admin || admin.role !== 'admin' || !(await bcrypt.compare(password, admin.password))) return res.status(401).json({ message: 'Invalid admin credentials' });
    if (admin.demotedByRole && admin.role !== 'admin') return res.status(403).json({ message: 'You have been demoted by the owner.' });
    if (admin.status === 'banned') return res.status(403).json({ message: 'Your account has been blocked.' });
...
app.post('/api/moderator/login', async (req, res) => {
    ...
    if (!moderator || moderator.role !== 'moderator' || !(await bcrypt.compare(password, moderator.password))) return res.status(401).json({ message: 'Invalid moderator credentials' });
    if (moderator.demotedByRole && moderator.role !== 'moderator') return res.status(403).json({ message: 'You have been demoted by the owner.' });
    if (moderator.status === 'banned') return res.status(403).json({ message: 'Your account has been blocked.' });
...
app.patch('/api/users', checkAdminAuth, async (req, res) => {
    const { id, name, email, role, status } = req.body;
    if (!id || !validateObjectId(id, res)) return;
    const user = await userModel.findById(id);
    if (!user) return res.status(404).json({ message: 'User not found' });
    const adminRank = getUserRank(req.user.role);
    const userRank = getUserRank(user.role);
    if (userRank >= adminRank && id !== req.session.userId.toString()) return res.status(403).json({ message: 'Cannot modify users with equal or higher rank' });
    if (id === req.session.userId.toString() && role && role !== user.role) return res.status(403).json({ message: 'You cannot change your own role' });
    const updateData = {};
    ...
    if (role && ['moderator', 'user', 'guest'].includes(role) && id !== req.session.userId.toString()) {
        if (getUserRank(role) >= adminRank) return res.status(403).json({ message: 'Cannot assign a role with equal or higher rank' });
        updateData.role = role;
        if (role === 'guest') {
...
app.patch('/api/users/demote', checkAdminAuth, async (req, res) => {
    const { id, fromRole } = req.body;
    if (!id || !fromRole || !['moderator'].includes(fromRole) || !validateObjectId(id, res)) return res.status(400).json({ message: 'Invalid id or fromRole' });
    const user = await userModel.findById(id);
    if (!user || user.role !== fromRole) return res.status(user ? 400 : 404).json({ message: user ? `User is not a ${fromRole}` : 'User not found' });
    const adminRank = getUserRank(req.user.role);
    const userRank = getUserRank(user.role);
    if (userRank >= adminRank) return res.status(403).json({ message: 'Cannot demote users with equal or higher rank' });
    await userModel.findByIdAndUpdate(id, {
        role: 'user',
        demotedBy: req.session.userId,
        demotedByRole: 'admin'
...
app.patch('/api/users/ban', checkAdminAuth, async (req, res) => {
    const { id } = req.body;
    if (!id || !validateObjectId(id, res)) return;
    const user = await userModel.findById(id);
    if (!user) return res.status(404).json({ message: 'User not found' });
    const adminRank = getUserRank(req.user.role);
    const userRank = getUserRank(user.role);
    if (userRank >= adminRank || id === req.session.userId.toString()) return res.status(403).json({ message: 'Cannot ban users with equal or higher rank or yourself' });
    if (user.status === 'banned') return res.status(400).json({ message: 'User is already banned' });
    await userModel.findByIdAndUpdate(id, {
        status: 'banned',
        bannedBy: req.session.userId,
...
app.patch('/api/users/unban', checkAdminAuth, async (req, res) => {
    const { id } = req.body;
    if (!id || !validateObjectId(id, res)) return;
    const user = await userModel.findById(id);
    if (!user) return res.status(404).json({ message: 'User not found' });
    const adminRank = getUserRank(req.user.role);
    const userRank = getUserRank(user.role);
    if (userRank >= adminRank || id === req.session.userId.toString()) return res.status(403).json({ message: 'Cannot unban users with equal or higher rank or yourself' });
    if (user.status !== 'banned') return res.status(400).json({ message: 'User is not banned' });
    await userModel.findByIdAndUpdate(id, {
        status: 'active',
        bannedBy: null,
...
app.delete('/api/users', checkAdminAuth, async (req, res) => {
    const { id } = req.body;
    if (!id || !validateObjectId(id, res)) return;
    const user = await userModel.findById(id);
    if (!user) return res.status(404).json({ message: 'User not found' });
    const adminRank = getUserRank(req.user.role);
    const userRank = getUserRank(user.role);
    if (userRank >= adminRank || id === req.session.userId.toString()) return res.status(403).json({ message: 'Cannot delete users with equal or higher rank or yourself' });
    await userModel.deleteOne({ _id: id });
    res.json({ message: 'User deleted' });
});
...
app.post('/api/profile/photo', checkAuth, upload.single('profilePhoto'), async (req, res) => {
    ...
    try {
        const photoUrl = `/uploads/${req.file.filename}?t=${Date.now()}`;
        const user = await userModel.findByIdAndUpdate(req.session.userId, { profilePhoto: photoUrl }, { new: true }).select('name email role profilePhoto');
        if (!user) return res.status(404).json({ message: 'User not found' });
        res.json({
...
app.post('/api/upload-profile-picture', checkAuth, upload.single('profilePhoto'), async (req, res) => {
    if (!req.file) return res.status(400).json({ message: 'No photo uploaded' });
    try {
        const photoUrl = `/uploads/${req.file.filename}?t=${Date.now()}`;
        const user = await userModel.findByIdAndUpdate(req.session.userId, { profilePhoto: photoUrl }, { new: true }).select('name email role profilePhoto');
...
app.get('/api/me', async (req, res) => {
    if (req.session.userId) {
        const user = await userModel.findById(req.session.userId).select('name email role profilePhoto');
...
app.post('/api/posts', checkAuth, upload.single('fileUpload'), async (req, res) => {
    const { content, linkInput, createdAt } = req.body;
    ...
    const populatedPost = await postModel.findById(newPost._id).populate('userId', 'name email profilePhoto');
    res.json({ message: 'Post created', post: populatedPost });
});

app.get('/api/posts', async (req, res) => {
    const posts = await postModel.find().populate('userId', 'name email profilePhoto').sort({ createdAt: -1 });
    res.json(posts);
});

app.get('/api/my-posts', checkAuth, async (req, res) => {
    try {
        const posts = await postModel.find({ userId: req.session.userId }).populate('userId', 'name email profilePhoto').sort({ createdAt: -1 });
        res.json(posts);
    } catch (error) {
...
app.patch('/api/posts', checkAuth, async (req, res) => {
    ...
    const post = await postModel.findOneAndUpdate({ _id: id, userId: req.session.userId }, updateData, { new: true }).populate('userId', 'name email profilePhoto');
...
app.get(['/', '/register', '/login', '/owner', '/admin', '/moderator'], async (req, res) => {
    ...
    if (req.path === '/') {
        ...
        if (req.session.userId) user = await userModel.findById(req.session.userId).select('name email role profilePhoto');
...
app.get('/reddit-posts', async (req, res) => {
    ...
    if (!/^[a-zA-Z0-9_]+$/.test(subreddit)) return res.status(400).json({ message: 'Invalid subreddit name' });
